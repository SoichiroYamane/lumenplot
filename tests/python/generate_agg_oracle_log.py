"""Generate and validate the fixed Matplotlib Agg base-10 log-frame fixture.
...
lookups, ``Axes.get_xscale``/``Axes.get_yscale``, and the public axes
transform.  The committed mask is derived from the reference image and the
fixture's resolved background/stroke colors; it never observes a
candidate/native render.

Run from the repository root with::

    PYTHONPATH=python:tests/python python tests/python/generate_agg_oracle_log.py
...
REFERENCE_PNG_PATH = FIXTURE_DIR / "log_axis_reference.png"
MASK_PATH = FIXTURE_DIR / "log_axis_mask.json"
...
# Five samples spanning ~2.6 decades on the log axis.  All values are finite
# and strictly positive so the base-10 log domain is well-defined; the
# invalid-domain clip rule (non-positive data) is exercised at the adapter
# spec level in the focused tests, not in this committed raster.
X_DATA = (0.1, 0.3, 0.5, 0.7, 0.9)
Y_DATA = (2.0, 12.0, 60.0, 250.0, 800.0)
...
def build_fixture_figure() -> tuple[Figure, Any, Line2D]:
    ...
    axes.add_line(line)
    # The log axis' limit clamp only applies once the scale is installed
    # (matplotlib axis.py ``_set_lim`` reads ``get_scale()``), so the scale
    # precedes the limit assignments: this is the real user order exercised
    # by the LP-FUNC-004 lane.
    axes.set_xscale(XSCALE)
    axes.set_yscale(YSCALE, base=LOG_BASE)
    axes.set_xlim(*XLIM)
    axes.set_ylim(*YLIM)
...
def public_line_topology(line: Line2D, axes: Any) -> dict[str, Any]:
    ...
    log_fractions = [
        (math.log10(float(y)) - math.log10(float(YLIM[0])))
        / (math.log10(float(YLIM[1])) - math.log10(float(YLIM[0])))
        for y in Y_DATA
    ]
    return {
        ...
        "segment_codes": codes,
        "vertices": vertices,
        "scales": {"x": XSCALE, "y": YSCALE, "log_base": LOG_BASE},
        "limits": {"xlim": list(XLIM), "ylim": list(YLIM)},
        "input_x": [float(value) for value in X_DATA],
        "input_y": [float(value) for value in Y_DATA],
        "display_api": "axes.transData.transform(data)",
        "display_vertices": display_vertices,
        "log_fractions_y": log_fractions,
    }
...
def render_reference() -> tuple[bytes, np.ndarray, dict[str, Any], tuple[int, int]]:
    ...
    with fixture_rc_context():
        ...
        topology = public_line_topology(line, axes)
...
def _manifest(
    *,
    ...
    return {
        ...
        "fixture_id": "log-axis-base10-line-decades-span",
        "oracle": {
