"""Generate and validate the fixed Matplotlib Agg stackplot fixture.

The reference is deliberately produced only with documented Matplotlib APIs:
``Figure``, ``Axes.stackplot``, ``FigureCanvasAgg.print_png``, public
font-manager lookups, and public collection ``get_paths`` getters. The
...
This fixture isolates ``Axes.stackplot`` (LP-FUNC-032 stack semantics,
LP-MPL-020 C2 stack surface) from the combined fill/fill-between/span
fixture in ``generate_agg_oracle_fill.py`` so stack composition parity is
attributable to the stack call alone: two opaque layers sharing one
baseline, rendered as two ``FillBetweenPolyCollection`` artists.

Run from the repository root with::

    PYTHONPATH=python:tests/python python tests/python/generate_agg_oracle_stackplot.py
...
REFERENCE_PNG_PATH = FIXTURE_DIR / "stackplot_reference.png"
MASK_PATH = FIXTURE_DIR / "stackplot_mask.json"
...
# Two non-overlapping opaque stack layers sharing one baseline.  Opaque faces
# keep the fixed pixel-class contract meaningful: interior pixels are
# byte-equal to the resolved face color, exactly like the combined fill
# lane's stack surface.
#
...
# the fixed gate allows -- the same rasterizer trait as the known line2d F1
# residual and the combined fill lane's slanted-fill scoping.  The
# pixel-parity fixture therefore pins the engine's exact surface; slanted
# stack pixel parity is follow-up work, not a widened gate.
...
# manifest.  They match the combined fill lane so the Agg behavior of the
# stack region is directly comparable across both fixtures.
...
def build_fixture_figure() -> tuple[Figure, Any, dict[str, Any]]:
    """Build the exact fixed Figure/stack input used by every test path."""
...
def public_stack_topology(artists: dict[str, Any]) -> dict[str, Any]:
...
def render_reference() -> tuple[bytes, np.ndarray, dict[str, Any], tuple[int, int]]:
    ...
    with fixture_rc_context():
        ...
        topology = public_stack_topology(artists)
...
def _manifest(
    *,
    ...
    return {
        ...
        "fixture_id": "stackplot-two-layer-opaque",
        "oracle": {
        ...
        "topology": topology,
        "mask_generation": (
            "reference-only: figure facecolor and resolved stack face "
