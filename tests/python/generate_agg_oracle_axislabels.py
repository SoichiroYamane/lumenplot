"""Generate and validate the fixed Matplotlib Agg axis-label/spine fixture.
...
tick/spine/axis-label/font-manager lookups, and ``Axis.get_ticklocs``. The
...
The frame carries exactly the B-2a (R2) surface: visible major tick
labels on both axes, the visible non-empty ``xlabel``/``ylabel`` pair,
the four visible spine edges of the axes box, major tick strokes, and
one axis-aligned filled polygon as the eligible content artist
(LP-FUNC-032 axis-aligned exact surface, so the only strokes in the
frame are the rectilinear decoration edges).

Run from the repository root with::

    PYTHONPATH=python:tests/python python tests/python/generate_agg_oracle_axislabels.py
...
REFERENCE_PNG_PATH = FIXTURE_DIR / "axislabels_reference.png"
MASK_PATH = FIXTURE_DIR / "axislabels_mask.json"
...
EFFECTIVE_DPI = 100.0
# The 200x100 frame must contain the axis labels' ink: unlike the
# tick-only fixture, the xlabel sits ~30px below the axes and the ylabel
# ~20px left of it, so the axes rect leaves a 47px bottom margin and a
# 44px left margin (both label extents verified inside the canvas).
AXES_RECT = (0.22, 0.47, 0.66, 0.43)
XLIM = (0.0, 10.0)
YLIM = (0.0, 5.0)
XTICKS = (0.0, 5.0, 10.0)
XTICKLABELS = ("0", "5", "10")
...
def public_axis_topology(axes: Any) -> dict[str, Any]:
    """Return tick/axis-label/spine/fill facts from public getters only."""
...
def _manifest(
    *,
    ...
    return {
        ...
        "fixture_id": "axis-labels-xlabel-ylabel-axes-box-spines",
        "oracle": {
        ...
        "figure": {
            ...
            "options": {
                ...
                "facecolor": "white",
                "edgecolor": "white",
                "axes_construction": "figure.add_axes((0.22, 0.47, 0.66, 0.43))",
