"""Generate and validate the fixed Matplotlib Agg center-title/spine fixture.
...
tick/spine/title/font-manager lookups, and ``Axis.get_ticklocs``. The
committed mask is derived from the reference image and the fixture's
resolved background/fill/spine colors; it never observes a
candidate/native render.

The frame carries exactly the B-2a (R3) surface: visible major tick
labels on both axes, the visible non-empty center ``title``, the four
visible spine edges of the axes box, major tick strokes, and one
axis-aligned filled polygon as the eligible content artist
(LP-FUNC-032 axis-aligned exact surface, so the only strokes in the
frame are the rectilinear decoration edges). No axis labels and no
legend appear in the frame, so the P3 legend-glyph quarantine cannot
interact with this fixture.

Run from the repository root with::

    PYTHONPATH=python:tests/python python tests/python/generate_agg_oracle_titles.py
...
EFFECTIVE_DPI = 100.0
# The 200x100 frame must contain the center title's ink: unlike the
# axis-label fixture, the title sits above the axes, so the axes rect
# leaves a 30px top margin (title extent verified inside the canvas)
# while the 44px left and 25px bottom margins keep the tick labels
# in-canvas.
AXES_RECT = (0.22, 0.25, 0.66, 0.45)
XLIM = (0.0, 10.0)
YLIM = (0.0, 5.0)
XTICKS = (0.0, 5.0, 10.0)
XTICKLABELS = ("0", "5", "10")
...
def public_axis_topology(axes: Any) -> dict[str, Any]:
    """Return tick/title/spine/fill facts from public getters only."""
    ...
    return {
        ...
        "title_center": axes.get_title("center"),
        "title_left": axes.get_title("left"),
        "title_right": axes.get_title("right"),
        "spines": spines,
        "polygons": polygons,
    }
...
def _manifest(
    *,
    ...
    return {
        ...
        "fixture_id": "center-title-axes-box-spines",
        "oracle": {
