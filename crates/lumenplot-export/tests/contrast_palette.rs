...
//
// Reads the frozen M5-C defaults in `lumenplot_engine::bridge` without
...
// compared UNROUNDED against the baselines. The values are a frozen public
// bridge surface and carry no legal WCAG conformance claim.
...
fn at_review_a11y_contrast_defaults_meet_baselines() {
    ...
    // 3:1 non-text baseline for every frozen series color, unrounded.
    assert!(
