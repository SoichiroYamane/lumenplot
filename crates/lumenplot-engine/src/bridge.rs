...
// M5-C frozen contrast palette (AT-REVIEW-A11Y, LP-UX-031; API-0004
// acceptance baselines 4.5:1 normal text / 3:1 large text / 3:1 non-text and
// focus indicators, compared unrounded). Frozen public bridge surface: the
// freeze review retires the provisional label for exactly these items, and
// these values carry no legal WCAG conformance claim. The cue vocabulary
...
// no silent fallback.
impl SrgbRgba8 {
    /// Frozen default background (opaque white).
    ...
    /// Frozen default text ink (opaque black). Covers both the 4.5:1
    ...
    /// Frozen default focus-indicator ink (opaque black, same value as
...
/// Frozen default series palette (opaque). Each entry clears the 3:1
/// non-text baseline against [`SrgbRgba8::DEFAULT_BACKGROUND`]; hues are
/// distinct but the set makes no color-vision-deficiency safety claim.
/// Pair with [`DEFAULT_SERIES_CUES`] so no series distinction relies on
/// color alone.
...
/// Frozen non-color cue vocabulary. One cue per [`DEFAULT_SERIES_PALETTE`]
/// entry, all pairwise distinct, so every series distinction carries at
/// least one non-color cue. Data-only in this slice: not consumed by any
/// renderer or export sink.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
#[non_exhaustive]
pub enum LineCue {
    Solid,
...
/// Frozen per-series cue assignment, index-aligned with
