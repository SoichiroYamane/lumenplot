...
| G-09 | Log-scale axes (`gs.label(xscale=...)`, legacy scale strings) | PARTIALLY (a)/(b)-adjacent | Linear is COVERED (`LP-FUNC-003`), log is COVERED by `LP-FUNC-004` (MUST, Ph 1) at the engine level, but the adapter currently refuses the *decorated* log axes case only because locators enable minor ticks. With `minorticks_off()` the log frame renders natively today (verified). Residual: locator-driven minor-tick policy decision + symlog/logit stay with LP-FUNC-039 (MAY, future). | `visible minor ticks are unsupported` (cleared by minorticks_off) | any scaled-axis example |
