...
[ADR 0003 — facade and crate DAG](../adr/0003-facade-and-crate-dag.md) records the accepted Option-C facade/crate split, [ADR 0010 — Phase-1 native core and facade contract](../adr/0010-phase1-native-core-facade-contract.md) records the exact first native core/facade slice, and [ADR 0011 — Phase-1B facade namespace and observation traits](../adr/0011-phase1b-facade-namespace-observation-traits.md) narrows its root namespace and observation guarantees. Phase-1A/B workspace implementation and local visibility/re-export, `publish = false`, and dependency-direction checks now have local evidence; later layers and product evidence remain pending. [ADR 0012 — private line frame and deterministic PNG contract](../adr/0012-private-line-frame-and-png-contract.md) records the bounded private Phase-2A/2B line/export boundary, whose implementation and local evidence do not expand the facade. [ADR 0013 — hidden line/PNG facade and private Python helper](../adr/0013-hidden-facade-private-python-line-png.md) records the staged Phase-3A owned seam and helper-only package boundary. [ADR 0014 — Phase-3A2 pinned manylinux wheel evidence](../adr/0014-phase3a2-pinned-manylinux-wheel-evidence.md) records the Phase-3A2 pinned builder and same-wheel matrix; Phase-3A2 helper/package/builder same-wheel evidence is recorded (CI-local manifest; GIL CPython 3.11–3.14). The public Phase-3B Matplotlib contract is recorded in [ADR 0015](../adr/0015-phase3b-public-matplotlib-adapter-contract.md) + [API 0005](api-0005-phase3b-public-matplotlib-backend-surface.md), and its first strict-mode and hybrid-explicit implementation slices are merged with local contract-test evidence while packaged public-backend runtime evidence is recorded in PR #89 CI. The conceptual layers and their allowed direction are fixed:
...
The internal RenderPacket is a validated process-local projection of that frame for a renderer instance. It is immutable and tied to a Scene revision/generation. It carries opaque logical resource identifiers rather than platform objects. [ADR 0004 — RenderPacket resource lifecycle](../adr/0004-renderpacket-resource-lifecycle.md) records the accepted internal packet/resource-lifecycle contract; bounded packet validation/generation and logical cache/lease/fence model evidence is recorded in PR #91, the shared semantic frame plus the validated owner handoff in PR #115, and bounded malformed/capacity packet bounds, exact generation-ordering properties, a session-layer generation-scoped cancellation pin, and the explicit EXIT-1 export not-to-wire record in PR #181 (merged as 1f263b7), while full semantic-frame integration, concrete renderer-owner wiring, LOD/layout worker binding, device-loss rebuild, and environment evidence remain pending.
...
The bounded Phase-3B Python implementation currently exposes the staged
constructor spellings `mode="strict"` and `mode="hybrid"`; `hybrid` is the
default and maps to `hybrid-explicit`. Canonical three-profile naming and the
`accelerated-native` end-to-end path remain M1/M6 closure work, not a support
claim for the current snapshot.
...
CI-local manifest with no public/support claim. That same-wheel evidence is now recorded:
the CI-local manifest captures the pinned-wheel build and the GIL CPython
3.11–3.14 cell executions, adding no platform or support claim. It has no Matplotlib
dependency, backend entry point, public result/diagnostic/warning/canvas API,
or fallback schema. The separate Phase-3B decision that records those public
surfaces now exists ([ADR 0015](../adr/0015-phase3b-public-matplotlib-adapter-contract.md)
+ [API 0005](api-0005-phase3b-public-matplotlib-backend-surface.md)), and its
first strict-mode and hybrid-explicit implementation slices are merged with
local contract-test evidence while packaged public-backend runtime evidence
remains pending.
...
O-01 through O-17 are now accepted contracts recorded in the linked ADR/API documents below. [ADR 0010](../adr/0010-phase1-native-core-facade-contract.md) resolves the Phase-1 native core/facade boundary and the API 0001/API 0002 candidates, and [ADR 0011](../adr/0011-phase1b-facade-namespace-observation-traits.md) narrows the Phase-1B namespace, token, and trait observations. Phase-1A/B implementation and local contract evidence now exist; their broader compatibility, platform, benchmark, product, and release evidence remains pending. [ADR 0012](../adr/0012-private-line-frame-and-png-contract.md) records the accepted private Phase-2A/2B line-frame and deterministic line/PNG contract, whose bounded implementation and local evidence remain private and do not close full-v1 export. [ADR 0013](../adr/0013-hidden-facade-private-python-line-png.md) records the staged Phase-3A hidden facade and private Python helper; [ADR 0014](../adr/0014-phase3a2-pinned-manylinux-wheel-evidence.md) records the Phase-3A2 pinned builder, same-wheel matrix, and CI-local evidence boundary. Phase-3A2 helper/package/builder same-wheel evidence is recorded (CI-local manifest; GIL 3.11–3.14); the public Phase-3B Matplotlib contract is recorded in [ADR 0015](../adr/0015-phase3b-public-matplotlib-adapter-contract.md) + [API 0005](api-0005-phase3b-public-matplotlib-backend-surface.md), and its first strict-mode and hybrid-explicit implementation slices are merged with local contract-test evidence while packaged public-backend runtime evidence is recorded in PR #89 CI. These records do not change the `Not implemented`, `Not measured`, or `environment required` statuses in the [traceability registry](../requirements/traceability-v1.0.md).

| Decision | Accepted record | Evidence boundary |
| --- | --- | --- |
| O-01 | [ADR 0003 — facade and crate DAG](../adr/0003-facade-and-crate-dag.md) + [ADR 0010 — Phase-1 native core/facade](../adr/0010-phase1-native-core-facade-contract.md) + [ADR 0011 — Phase-1B namespace and observations](../adr/0011-phase1b-facade-namespace-observation-traits.md) + [ADR 0013 — Phase-3A hidden facade](../adr/0013-hidden-facade-private-python-line-png.md) | Local Phase-1 static DAG, visibility/root allowlist, `publish = false`, and dependency-direction guards have implementation/test evidence; Phase-3A hidden inventory is conditional until implementation; broader later-crate/backend/publication and full-v1/release evidence remains pending |
| O-02R/O-05 | [API 0001 — native Scene, view, and owned data](api-0001-native-scene-state.md) + [ADR 0010 — Phase-1 native core/facade](../adr/0010-phase1-native-core-facade-contract.md) + [ADR 0011 — Phase-1B namespace and observations](../adr/0011-phase1b-facade-namespace-observation-traits.md) | Local Phase-1 Scene transaction/revision/identity/snapshot fixtures exist; bounded frame/packet validation, generation, and logical resource-lifecycle model tests exist, while the shared semantic frame, concrete renderer-owner integration, async/runtime device-loss rebuild, and full-v1 lifecycle evidence remain pending |
| O-03 | [API 0002 — errors, capabilities, and fallback](api-0002-errors-capabilities-fallback.md) + [ADR 0010 — Phase-1 native core/facade](../adr/0010-phase1-native-core-facade-contract.md) + [ADR 0011 — Phase-1B namespace and observations](../adr/0011-phase1b-facade-namespace-observation-traits.md) + [ADR 0013 — Phase-3A bridge mapping](../adr/0013-hidden-facade-private-python-line-png.md) | Local Phase-1 error/token/exhaustive engine-to-facade mapping fixtures exist; Phase-3A error mapping is recorded but unimplemented; backend/runtime/Python/fallback capability and full-v1 error matrix evidence remain pending |
| O-04 | [ADR 0004 — RenderPacket resource lifecycle](../adr/0004-renderpacket-resource-lifecycle.md) | PR #91 records bounded logical cache keying, packet leases, completion-fence retirement, multiple-owner accounting, and device-generation invalidation tests; PR #115 adds the shared semantic frame, validated owner handoff, and retained CPU-frame rebuild; PR #181 (merged as 1f263b7) adds bounded malformed/capacity packet bounds, exact generation-ordering properties, a session-layer generation-scoped cancellation pin, and the explicit EXIT-1 export not-to-wire record; full renderer-owner integration, LOD/layout worker binding, device-loss rebuild, and environment evidence remain pending |
| O-06 | [ADR 0005 — runtime, viewer, and host loop](../adr/0005-runtime-viewer-host-loop.md) | Backend-neutral runtime/viewer lifecycle and semantic-input state-model tests are recorded; real window/present and platform lifecycle matrix remain pending |
...
| O-02P/O-09/O-10 | [API 0003 — Phase-3A Python, NumPy, and private helper](api-0003-python-numpy-matplotlib.md) + [ADR 0013 — hidden facade/private Python helper](../adr/0013-hidden-facade-private-python-line-png.md) + [ADR 0014 — pinned wheel evidence](../adr/0014-phase3a2-pinned-manylinux-wheel-evidence.md) + [ADR 0015 — Phase-3B public adapter contract](../adr/0015-phase3b-public-matplotlib-adapter-contract.md) + [API 0005 — Phase-3B public backend surface](api-0005-phase3b-public-matplotlib-backend-surface.md) | Phase-3A2 builder, same-wheel, and manifest contract recorded; helper/package/builder same-wheel and four-cell runtime execution results recorded (CI-local manifest; no platform/support claim); Phase-3B strict/hybrid implementation slices and packaged public-backend runtime evidence are recorded in PR #89 CI; full profile/compatibility/platform/release closure remains pending |
...
- a completed v1 Rust engine or viewer (the bounded Phase-1A kernel/Phase-1B facade and runtime/viewer state model have local implementation evidence);
...
- a finished PNG/PDF/SVG exporter (bounded private line PNG/PDF sinks exist, but full semantic/text/export closure does not);
- broad packaged public-backend compatibility evidence for the Matplotlib adapter (PR #89 records the installed-wheel/runtime smoke evidence; full support and matrix claims remain open);
