...
This is the public high-level architecture companion to the accepted pre-alpha requirements and ADR 0002. Phase-1A/B native implementation and local contract evidence now exist, and the bounded Phase-2A/2B line-frame and PNG implementation has local evidence. PR #89 also records bounded RenderPacket validation, offscreen wgpu line/shader/readback tests, runtime/viewer/input state-model tests, a private line-only PDF sink, and fail-closed benchmark/wheel evidence; publication, full-v1 product completion, platform, benchmark, support, and compatibility evidence remain open.
...
The internal RenderPacket is a validated process-local projection of that frame for a renderer instance. It is immutable and tied to a Scene revision/generation. It carries opaque logical resource identifiers rather than platform objects. [ADR 0004 — RenderPacket resource lifecycle](../adr/0004-renderpacket-resource-lifecycle.md) records the accepted internal packet/resource-lifecycle contract; bounded packet validation/generation evidence is recorded, while full semantic-frame integration, cache/lease/fence-retirement ownership, and device-loss renderer integration remain pending.
...
| O-02R/O-05 | [API 0001 — native Scene, view, and owned data](api-0001-native-scene-state.md) + [ADR 0010 — Phase-1 native core/facade](../adr/0010-phase1-native-core-facade-contract.md) + [ADR 0011 — Phase-1B namespace and observations](../adr/0011-phase1b-facade-namespace-observation-traits.md) | Local Phase-1 Scene transaction/revision/identity/snapshot fixtures exist; bounded frame/packet validation and generation tests exist, while the shared semantic frame, full resource lifecycle, async/runtime/device generations, and full-v1 lifecycle evidence remain pending |
...
| O-04 | [ADR 0004 — RenderPacket resource lifecycle](../adr/0004-renderpacket-resource-lifecycle.md) | Bounded packet validation/generation and resource-lease model tests are recorded; full renderer-owner cache/lease/fence integration and device-loss evidence remain pending |
