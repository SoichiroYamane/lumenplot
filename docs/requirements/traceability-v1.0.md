...
| M2.5 bounded 3D native/adapter parity slice | [ADR 0016](../adr/0016-v1-3d-envelope-and-agg-parity.md) + `tests/python/test_agg_oracle_3d.py` + render-api owner-boundary tests | Three pinned perspective/orthographic fixtures, public `Line3D`/`Poly3DCollection` preflight, fixed-cell triangle/stroke parity, explicit refusal/fallback, and additive semantic-frame evidence pass in PR #116 CI; full requirement, platform matrix, and release closure remain open |
...
| `LP-FUNC-025` | `MUST` | pinned `FigureCanvasAgg` mplot3d four-part parity, including explicit projection/view attributes, canonical f64 x/y/z bounds, projected line/triangle geometry, and reference depth-order artifacts | 3 | v1 | `AT-FUNC-3D` | Not implemented (bounded M2.5 native/adapter contract evidence: PR #116; `TestCommitted3DOracleFixtures` regenerates and validates three pinned perspective/orthographic fixtures, `Test3DAdapterNativeSpec` checks canonical f64 bounds/origin, projected line/triangle style/order, and `Test3DRefusalAndFallback` checks strict refusal plus one whole-frame hybrid Agg fallback; the CPython 3.11 and 3.13 CI runs pass these fixtures, while full four-part v1 closure, platform matrix, and release claim remain open) |
