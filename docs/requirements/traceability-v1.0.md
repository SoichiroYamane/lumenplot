...
- The 2026-09-11 window-app scope proposal (pending maintainer acceptance, scope card `t_53261829`) adds `LP-FUNC-042` for the interactive window application (native event loop, window/present, close) as a Phase 3 v1 `MUST` under the proposed [ADR 0005](../adr/0005-runtime-viewer-host-loop.md) amendment note. It remains honestly `environment required — Not implemented`; the M4-B1 private headless seams and the headless witness recorded on `LP-PLAT-010`/`LP-QUAL-020` are partial evidence only, and no support, performance, or release claim is made.
...
The 2026-09-11 window-app scope proposal (pending maintainer acceptance) adds one
...
| `LP-FUNC-042` | `MUST` | `AT-FUNC-VIEWER`, `AT-LIFE-VIEWER`, `AT-LIFE-RUNTIME` | environment required — Not implemented (proposal t_53261829, pending maintainer acceptance; bounded partial evidence only; product launch, repeated cycles, real present, declared-cell matrix, and the ADR-0005 remainder still open; no support, performance, or release claim) |
...
- The 2026-09-11 window-app scope proposal adds `LP-FUNC-042` (interactive window application: native event loop, window/present, close; Phase 3 v1 `MUST`; `AT-FUNC-VIEWER`, `AT-LIFE-VIEWER`, `AT-LIFE-RUNTIME`) as pending scope; it complements `LP-FUNC-016` and `LP-PLAT-009`/`LP-PLAT-010` without changing any existing row, and its transport/host scope (declared-cell-first vs matrix) plus Q1–Q5 remain open questions in the proposed ADR 0005 note and the M4 roadmap placement.
