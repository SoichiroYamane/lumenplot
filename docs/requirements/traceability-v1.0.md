...
- The 2026-09-05 envelope amendment promotes `LP-FUNC-025` to a Phase 3 v1 `MUST` under [ADR 0016](../adr/0016-v1-3d-envelope-and-agg-parity.md). It remains honestly `Not implemented` as a full v1 requirement; the bounded, CI-verified M2.5 native/adapter evidence from PR #116 is recorded below, and whole-frame Agg fallback alone is not native 3D evidence.
- The 2026-09-11 window-app scope proposal, accepted 2026-09-13 (maintainer approval; scope card `t_53261829`, acceptance `t_2db160d8`), adds `LP-FUNC-042` for the interactive window application (native event loop, window/present, close) as a Phase 3 v1 `MUST` under the accepted [ADR 0005](../adr/0005-runtime-viewer-host-loop.md) amendment note. It remains honestly `environment required — Not implemented`; the M4-B1 private headless seams and the headless witness recorded on `LP-PLAT-010`/`LP-QUAL-020` are partial evidence only, and no support, performance, or release claim is made.
...
Adopted 2026-08-26 with the GAP-R1 candidate rows ([`codex-gap-verification-candidate-rows.md`](../research/codex-gap-verification-candidate-rows.md)):

| Gate family | Evidence expected |
| --- | --- |
| `AT-FUNC-NAN-GAP` | Deterministic functional test of non-finite sample fidelity (NaN/masked/infinite gaps in eligible line data) against the Agg oracle. |
| `AT-MPL-PREFLIGHT-SOUNDNESS` | Matplotlib strict-mode soundness test: every unrepresentable Figure (ineligible scale class, inverted limits, zero projected content) is refused explicitly with no PNG. |
| `AT-MPL-UNIT-DATA` | Matplotlib adapter test of publicly converted unit/date data driving eligible-line geometry (`orig=False` view) with explicit refusal of unrepresentable conversions. |
...
| v1 3D envelope and Agg-parity gate | [ADR 0016](../adr/0016-v1-3d-envelope-and-agg-parity.md) | Accepted product envelope and O-19–O-22 amendment (PR #114); bounded M2.5 native/adapter evidence is recorded below; full `LP-FUNC-025` remains `Not implemented` |
| M2 semantic frame and RenderPacket owner lifecycle | [ADR 0004](../adr/0004-renderpacket-resource-lifecycle.md) + render-api/runtime owner tests | PR #115 CI verifies the shared `SemanticFrame`, doc-hidden validated owner handoff, distinct scene/work/device generations, resource lease/fence ownership, and retained CPU-frame rebuild; PR #181 (merged as 1f263b7) adds bounded malformed/capacity packet-bounds tests, exact generation-ordering/triple-matching properties, a session-layer generation-scoped cancellation pin, and the explicit EXIT-1 not-to-wire record for export; broader packet/resource/lifecycle and environment evidence remain open |
| M2.5 bounded 3D native/adapter parity slice | [ADR 0016](../adr/0016-v1-3d-envelope-and-agg-parity.md) + `tests/python/test_agg_oracle_3d.py` + render-api owner-boundary tests | Four pinned perspective/orthographic fixtures, public `Line3D`/`Poly3DCollection` preflight, fixed-cell triangle/stroke parity, explicit refusal/fallback, and additive semantic-frame evidence pass in PR #116 CI with the merged PR #166 `line3d_orthographic` fixture evidence (`test_orthographic_line_fixture_records_manifest_input_and_topology`, `test_line3d_orthographic_spec_keeps_f64_source_and_gap_semantics`); full requirement, platform matrix, and release closure remain open |

## Coverage summary

- Requirement entries: **238**.
- Normative `MUST`/`MUST NOT` entries requiring closure: **158**.
- Classification counts: MAY=9, MUST=121, MUST NOT=37, NON-GOAL=5, PHASE=6, REFERENCE=13, SHOULD=47.
- Stable families: `DATA` (10), `EXPORT` (10), `FUNC` (42), `LOD` (7), `MPL` (23), `PERF` (16), `PLAT` (12), `PROD` (18), `QUAL` (28), `REL` (14), `RENDER` (10), `SEC` (8), `TEXT` (7), `UX` (33).
...
The 2026-09-11 window-app scope proposal, accepted 2026-09-13, adds one
requirement row, `LP-FUNC-042` (`MUST`, reusing the existing `AT-FUNC-VIEWER`,
`AT-LIFE-VIEWER`, and `AT-LIFE-RUNTIME` gates with no new gate name): the controlled
totals change from 237 to 238 entries and from 157 to 158 normative entries, with
distinct gates unchanged at 107. Its result is honestly `environment required —
Not implemented`.
...
| `LP-FUNC-003` | `MUST` | axis, tick, and label correctness | 1 | v1 | `AT-FUNC-AXIS` | Not implemented (bounded Phase-3B local contract evidence: PR #63; native major-gridline/major-tick/spine rendering for eligible decorated `axison=True` axes per ADR 0015 §4a — `TestDecoratedAxesSpec`, `TestDecoratedAxesEligibility` in `test_phase3b_backend.py`; date formatting remains unclosed; PR207 extension (B-2a R2): `TestAxisLabelWhitelist`, `TestAxisLabelDrawOrder`, `TestAxisLabelStyleContract` in `tests/python/test_phase3b_axislabels.py` plus `TestCommittedAxisLabelFixture` and `TestAxisLabelAdapterSemantics` in `tests/python/test_agg_oracle_axislabels.py` with `tests/python/generate_agg_oracle_axislabels.py --check`, fixture `axis-labels-xlabel-ylabel-axes-box-spines` under the UNCHANGED fixed S15.1 contract (background byte-equal, fully-covered byte-equal, fringe max channel delta 1, total-pixel fringe mismatch rate <= 0.001), deliberately no strict native pixel gate) |
| `LP-FUNC-004` | `MUST` | log-axis behavior | 1 | v1 | `AT-FUNC-LOG-AXIS` | Implemented (bounded Phase-3B local contract evidence: W3 lane t_a239680f; strict-mode base-10 log axes per LP-MPL-020 — scale dispatch accepting `linear`/base-10 `log` and refusing other scales explicitly, scale-consistent projection for content/fills/gridlines/tick strokes verified against `Axes.transData`, non-positive data following matplotlib's clip rule (`out[values <= 0] = -1000` in log units), invalid view domains inherited from matplotlib's own limit clamp, default mathtext labels refusing via the existing T-lane gate while plain formatters render natively — `TestLogAxisStrictEligibility`, `TestLogProjection`, `TestLogAxisRefusals`, `TestLogFramePixelFixtures` in `test_phase3b_log_axis.py`; stroke-edge parity within the ratified band (plain 0.9777, decorated 0.9695 within32); non-base-10 log scales and symlog/logit stay with LP-FUNC-039, any release claim stays open; PR153 extension: `TestCommittedLogAxisFixture` and `TestLogAxisNativeAggParity` in `tests/python/test_agg_oracle_log.py` plus `tests/python/generate_agg_oracle_log.py --check`, fixture `log-axis-base10-line-decades-span`, fixed S15.1 contract (background byte-equal, fully-covered byte-equal, fringe max channel delta 1, total-pixel fringe mismatch rate <= 0.001), joined-oblique route b2004ee, merge f3bd279, CI 7/7) |
...
| `LP-FUNC-012` | `MUST` | annotation acceptance fixture | 2 | v1 | `AT-FUNC-ANNOTATION`, `AT-EXPORT-ANNOTATION` | Not implemented (bounded M5-ANNOT-1B/2 local contract evidence: live Data2D rectangle + identity-text mirror — `scene/annotation_slice.rs` filter plus hit path, `PlotLayout::from_live_parts`, frame mirror selection, `annotation_slice1` engine/export suites incl. fixture text-box PNG ink; Slice-3 extension: live Data2D identity lines + arrows mirror (merge c219cd15, PR #198) — same-crate filter admits Data2D identity lines/arrows in id order, `from_live_parts` gates identically, frame selection untouched, `annotation_slice1` PNG pins line-shaft/arrow-shaft ink; Slice-4 extension: live AxesLogical identity 4-kind mirror (merge 9baa0ae, PR #200) — same-crate filter admits AxesLogical identity rectangles/text/lines/arrows in id order, `from_live_parts` gates identically, frame selection untouched, `annotation_slice1` suites pin extended admission predicate plus AxesLogical line-shaft ink; Slice-5 extension: live FigureLogical/DisplayLogical identity 4-kind mirror (merge ae35eef, PR #206) — same-crate filter admits FigureLogical/DisplayLogical identity rectangles/text/lines/arrows in id order, `from_live_parts` gates identically, frame selection untouched, `annotation_slice1` suites pin extended admission predicate plus FigureLogical/DisplayLogical ink; remaining kinds, spaces, and release closure stay open) |
...
| `LP-FUNC-025` | `MUST` | pinned `FigureCanvasAgg` mplot3d four-part parity, including explicit projection/view attributes, canonical f64 x/y/z bounds, projected line/triangle geometry, and reference depth-order artifacts | 3 | v1 | `AT-FUNC-3D` | Not implemented (bounded M2.5 native/adapter contract evidence: PR #116 with the merged PR #166 `line3d_orthographic` fixture; `TestCommitted3DOracleFixtures` regenerates and validates four pinned perspective/orthographic fixtures including `test_orthographic_line_fixture_records_manifest_input_and_topology`, `Test3DAdapterNativeSpec` checks canonical f64 bounds/origin, projected line/triangle style/order including `test_line3d_orthographic_spec_keeps_f64_source_and_gap_semantics`, and `Test3DRefusalAndFallback` checks strict refusal plus one whole-frame hybrid Agg fallback; the CPython 3.11 and 3.13 CI runs pass these fixtures, while full four-part v1 closure, platform matrix, and release claim remain open) |
...
| `LP-PROD-011` | `MUST` | architecture review | 0 | v1 architecture | `AT-REVIEW-FRAME-BOUNDARY` | Implemented (bounded M2 local contract evidence: PR #115; `SemanticFrame` is a distinct shared semantic/layout source for the validated `RenderPacket`, and `packet::tests::independent_consumers_read_one_validated_semantic_frame` passes in the CI Rust gate; broader semantic-frame integration remains open) |
| `LP-PROD-012` | `MUST` | schema and visibility review | 0 | v1 architecture | `AT-REVIEW-PACKET` | Implemented (bounded M2 local contract evidence: PR #115; the doc-hidden `__internal` owner seam, immutable whole-packet validation, distinct scene/work/device generations, stale rejection, and root-export/public-constructor guards pass in CI; full property, renderer, and environment coverage remain open) |
...
| `LP-PLAT-001` | `MUST` | portable backend build and runtime | 1 | v1 | `AT-LIFE-PLATFORM` | Implemented (bounded M3 environment-cell evidence: single-host observation, not CI; the portable wgpu/Vulkan backend builds and the offscreen validated-owner harness `validated_owner_harness_covers_oracle_scales` passes on the Radeon RADV PHOENIX real-GPU executed cell and the Lavapipe control cell, headless, across the 1x/1.25x/2x/3x oracle-scale/resource-reuse loop with validated-submission geometry and no decoded-pixel comparison, independently re-executed with exit 0 on both cells; numeric pixel tolerance stays OPEN, and window/present, main-thread lifecycle, device-loss/OOM matrix, multi-cell support-matrix, performance, and release claims remain open) |
...
| `LP-PLAT-010` | `MUST` | lifecycle matrix | 1-3 | v1 | `AT-LIFE-RUNTIME` | environment required — Not implemented (bounded witness only: t_fafb7de4 headless 12-row logical outcomes plus 7 shm health transients at 34753814 per t_8a014b73 audit; status unchanged; product launch, repeated cycles, real present, and ADR-0005 remainder still open; no support, performance, or release claim) |
| `LP-QUAL-009` | `REFERENCE` | candidate review | 1 | reference | `AT-SEC-SHADER` | Reference only — no support or implementation claim |
| `LP-QUAL-010` | `MUST` | reproducible artifact build | 1 | v1 | `AT-SEC-SHADER` | Implemented (bounded M3 Phase-B local contract evidence: PR #118; `build.rs` parses and fully validates the line-only WGSL artifact at build time with all eight manifest fields pinned fail-closed, and `manifest_metadata_is_authoritative_and_fail_closed` passes in the CI Rust gate; generated native-form and multi-artifact-class coverage remain open) |
| `LP-QUAL-011` | `MUST` | artifact manifest | 1 | v1 | `AT-SEC-SHADER` | Implemented (bounded M3 Phase-B local contract evidence: PR #118; `shaders/manifest.toml` is authoritative for schema/artifact/source_revision/sha256/validator/validation/resource_layout/coordinate_space with the `src/shader.rs` runtime linkage refusing on drift, verified by `manifest_metadata_is_authoritative_and_fail_closed` in the CI Rust gate; multi-artifact provenance remains open) |
| `LP-QUAL-012` | `MUST` | validator corpus | 1 | v1 | `AT-SEC-SHADER` | Implemented (bounded M3 Phase-B local contract evidence: PR #118; the single declared line artifact class passes naga full-capability validation at build time and again via `verify_line_shader_artifact` before any GPU use, pinned by the manifest `validator`/`validation` fields in the CI Rust gate; additional artifact classes remain open; the declared line class additionally executes on the Radeon RADV PHOENIX executed cell and the Lavapipe control cell via the offscreen validated-owner harness with validated-submission geometry only, while numeric pixel tolerance stays OPEN) |
...
| `LP-SEC-003` | `MUST NOT` | runtime negative test | 1 | v1 | `AT-SEC-SHADER` | Implemented (bounded M3 Phase-B local contract evidence: PR #118; `static_runtime_download_negative_guard` passes in the CI Rust gate — the line-only runtime surface carries no network/filesystem/process shader fetch or compile path and embeds the artifact via `include_str!`; broader runtime-surface coverage remains open) |
| `LP-SEC-004` | `MUST` | malformed-input tests | 0-1 | v1 | `AT-SEC-SIZES` | Not implemented (bounded M2 local contract evidence: PR #181, merged as 1f263b7; `packet::tests::malformed_canvas_metadata_and_pixel_capacity_fail_before_allocation` pins zero/over-dimension canvas, over-budget pixels through the checked integer path, non-finite and non-positive metadata, and out-of-canvas plot geometry, and `packet::tests::series_and_point_capacity_bounds_are_exact` pins the exact inclusive series bound plus one-past failure and point-count overflow before draw construction, passing in the CI Rust gate; allocation/upload/indexing-path coverage beyond the packet builder, the segment-count bound above 1M, and full v1 closure remain open) |
...
| `LP-MPL-003` | `MUST` | profile selection tests | 1-2 | v1 | `AT-MPL-PROFILES` | Implemented (bounded Phase-3B local contract evidence: `TestProfileSelectionValues`, `TestStagedDefaultProfile`, `TestSelectionIsImmutable`, `TestSelectionDrivesDispatch`; the shipped slice pins `strict`/`hybrid` selection with `hybrid` as the default mapping to `hybrid-explicit`, explicit rejection of every other value including deferred `accelerated-native`, and the new default-dispatch coverage; canonical profile-name closure and accelerated-native delivery stay open) |
...
| `LP-QUAL-018` | `MUST` | cancellation and queue tests | 0-1 | v1 architecture | `AT-LIFE-CANCELLATION` | Not implemented (bounded M2 local contract evidence: PR #181, merged as 1f263b7; `runtime::tests::generation_scoped_cancellation_keeps_current_work_and_counts_drops` pins session-layer generation-scoped cancellation — `cancel_except` preserves current work, `cancel_all` drains with full accounting, evicted tickets report stale and never ready — passing in the CI Rust gate; scoped per the accepted row-state recommendation, LOD/layout worker binding stays open and full v1 closure remains open) |
| `LP-QUAL-019` | `MUST NOT` | reentrancy test | 1-2 | v1 | `AT-LIFE-FFI` | Not implemented |
| `LP-QUAL-020` | `MUST` | device-loss/OOM matrix | 1-3 | v1 | `AT-LIFE-RUNTIME` | Implemented (bounded M2 local contract evidence: PR #115; `PacketOwner` retains the CPU semantic frame and rebuilds owner packets after device-generation invalidation, with `tests::packet_owner_retains_cpu_frame_and_rebuilds_after_device_invalidation` passing in the CI Rust gate; environment-backed device/surface/present coverage and full v1 lifecycle closure remain open) (bounded witness extension: t_fafb7de4 headless TerminalOutOfMemory mapping plus DeviceLost and RecoveryFailed explicit outcomes at 34753814 per t_8a014b73 audit; status unchanged; environment-backed device and surface remainder still open; no support, performance, or release claim) |
...
| `LP-FUNC-032` | `MUST` | fill geometry and span fixtures — quality oracle: current Matplotlib Agg backend output; `AT-FUNC-FILL` is met only when Agg parity fixtures pass (§5.4): geometry parity to sub-pixel rounding including span baselines, decoded-pixel difference within recorded thresholds, style semantics equal to Agg's resolution | 3B-cont. | v1 | `AT-FUNC-FILL` | Implemented (bounded Phase-3B local contract evidence: PR #68; `TestFillWhitelist`, `TestFillCollectorTrace`, `TestFillStyleContract`, `TestFillPixelParity` in `test_phase3b_fill.py` — strict eligibility for `Polygon` (`Axes.fill`) and `FillBetweenPolyCollection` (`Axes.fill_between`) under the opt-in `agg_srgb` blend mode, §5.4 Agg-oracle parity fixtures pass within recorded thresholds; `stackplot` composition and any release claim stay open; extended by PR #161 stackplot-oracle fixture evidence (merged as c7f9f13, CI 7/7): `TestCommittedStackplotFixture`, `TestStackplotCollectorTrace`, `TestStackplotStyleContract`, `TestStackplotNativeAggParity` in `test_agg_oracle_stackplot.py` with generator `--check`, fixture_id `stackplot-two-layer-opaque`, fixed S15.1 contract (background and fully-covered byte equality, fringe max channel delta 1, total-pixel fringe mismatch rate <= 0.001); slanted-edge parity and any release/support/performance/platform/WCAG claim stay open) |
| `LP-FUNC-033` | `MUST` | bar geometry and stacking fixtures — quality oracle: current Matplotlib Agg backend output; `AT-FUNC-BAR` is met only when Agg parity fixtures pass (§5.4): anchored-baseline geometry (negative and stacked included) to sub-pixel rounding, pixel parity within recorded thresholds, style semantics equal to Agg's resolution | 3B-cont. | v1 | `AT-FUNC-BAR` | Implemented (bounded Phase-3B local contract evidence: PR #69; `TestBarWhitelist`, `TestBarCollectorTrace`, `TestBarStyleContract`, `TestBarPixelParity`, `TestBarModeBehavior` in `test_phase3b_bar.py` — axis-aligned `Rectangle` bars anchored to declared baselines incl. negative, stacked, and `bottom=` cases, interiors byte-exact and edges inside the ratified 32/255 AA quantum vs Agg; rotated rectangles stay refused; extended by PR #89 eligibility surface, PR #102 pinned Agg-oracle bar fixtures (`TestCommittedBarFixture`, `TestBarNativeAggParity` in `test_agg_oracle_bar.py`, merged as 24bde5a with the strict native parity gate green in CI — exact classes byte-exact, i.e. exact-class 0 mismatches under the oracle contract), and PR #107 24.8 rect-ring convergence (1031->747; fringe 997->713, 34-exact retained; AA-FMT whitespace-only); adapter-scoped only, native Phase-1 rows stay open per §15.1 dual meaning; any release claim stays open) |
| `LP-FUNC-034` | `MUST` | drawstyle equivalence fixtures against Agg — quality oracle: current Matplotlib Agg backend output; `AT-FUNC-DRAWSTYLE` is met only when steps-pre/post/mid fixtures pass (§5.4): exact generated vertex sequences, pixel parity within recorded thresholds, style semantics equal to Agg's resolution | 3B-cont. | v1 | `AT-FUNC-DRAWSTYLE` | Implemented (bounded Phase-3B local contract evidence: PR #70; `TestStepGeometryParity`, `TestStepStyleContract`, `TestStepTopologyRecurrence`, `TestStepPixelParity`, `TestStepNonFiniteFallback`, `TestStepProfileModes` in `test_phase3b_steps.py` — exact `cbook`-parity vertex generation for steps-pre/post/mid plus the `steps` alias, stroke-edge pixel parity ≥95% within tol=32 with worst ≤228 measured, non-finite samples refusing explicitly; extended by PR #111 steps convergence (c17ab3e, merged as 4d49217, CI green): rectilinear open-chain step strokes route to the 24.8 cell integrator with the existing snap rule and a JOINED miter outline, resolving the 1598/max-228 parity residual baselined on the still-open PR #109 steps-oracle branch (68ddac3, fixture-only and superseded — never merged, landed evidence lives in PR #111); markers and unknown drawstyles stay refused, any release claim stays open) |
| `LP-FUNC-035` | `SHOULD` | cross-primitive ordering and alpha golden tests — quality oracle: current Matplotlib Agg backend output; `AT-SEM-COMPOSITING` is met only when golden frames match Agg layering/occlusion across mixed primitive classes within recorded thresholds (§5.4) | 3B-cont.+1 | v1 quality | `AT-SEM-COMPOSITING` | Not implemented (bounded Phase-3B local contract evidence: PR #75; `TestCompositingSpecOrder`, `TestCompositingAlphaSpecOrder`, `TestCompositingHybridAndRefusal`, `TestAxesPatchExclusion`, `TestClassMixedTieOrder`, `TestCompositingPixelParity`, `TestByteExactScenes`, `TestZorderMatrix` in `test_phase3b_compositing.py` — Agg stable-sort ordering across mixed primitive classes incl. ties and patch exclusion, alpha both-directions pixel parity within the ratified 32/255 AA band, byte-exact bar/opaque/disjoint-alpha scenes, class x zorder matrix; slanted-translucent-over-AA-edge scenes stay excluded, tick-label surface stays frozen out, every-output-path and release claims stay open) |
...
| `LP-FUNC-037` | `SHOULD` | date-tick formatting fixtures — quality oracle: current Matplotlib Agg backend output; `AT-FUNC-DATE-AXIS` is met only when label fixtures pass (§5.4): locator/formatter results equal to Agg's and glyph outlines matching Matplotlib `TextPath` output for the same font properties | 3B-cont.+1 | v1 quality | `AT-FUNC-DATE-AXIS` | Not implemented (bounded Phase-3B local contract evidence: PR #74; `TestDateAxisStrictEligibility`, `TestUnitConversionUpstream`, `TestDateAxisRefusals`, `TestDateFramePixelParity` in `test_phase3b_date_axis.py` — date frames render strict through the public `orig=False` unit route with view-interval label enumeration equal to the drawn stream, formatter strings carried per glyph command, explicit usetex refusal with hybrid whole-frame fallback, pinned-frame pixel parity gated at >=95% within 32 (worst cap 255) plus the default-frame structural gate; axis-label surface strict-eligible per PR207 (B-2a R2, same class/test identifiers as above); `TextPath`-outline equality for date ticks and release claims stay open) |
...
| `LP-MPL-020` | `SHOULD` | eligibility-extension process review — every whitelist growth lands its four mechanics together (whitelist entry, collector-trace expectation, style contract, fixtures) and those fixtures must include the §5.4 Agg-oracle parity criteria for the new artist class | continuous | v1 governance | `AT-MPL-ELIGIBILITY` | Not implemented (bounded eligibility-extension evidence: PR #167, merged as dd00b19 with head 140332fd66a17250a96ac1484b803dc852b8a4ce, PR checks 7/7 — `TestCommittedLegendFixture`, `TestLegendAdapterSemantics`, `TestLegendNativeAggParity` in `test_agg_oracle_legend.py` with fixture_id `legend-line-handles-framed-single-column` under the fixed S15.1 contract (background byte-equal, fully-covered byte-equal, fringe max channel delta 1, total-pixel fringe mismatch rate <= 0.001); the P3-owned glyph quarantine pins the two Agg-measured label boxes verbatim with a fixed 2 px pad, and the parked-residual assertion fails LOUD when P3 clears the glyph residual, forcing the quarantine's removal; the full requirement, any release claim, and LP-FUNC-009 / LP-UX-016..022 stay open; PR207 extension (B-2a R2, second four-mechanics exemplar alongside PR #167): same class/test/fixture identifiers as above; the row stays Not implemented (governance row; exemplars do not close it), P3 glyph quarantine and LP-FUNC-009 / LP-UX-016..022 notes unchanged) |
| `LP-FUNC-040` | `MUST` | non-finite sample fidelity fixtures — quality oracle: current Matplotlib Agg backend output; `AT-FUNC-NAN-GAP` is met only when gap fixtures (interior NaN, leading/trailing non-finite, masked arrays, infinities) pass (§5.4): both segments adjacent to a non-finite sample are omitted, painting resumes at the next finite sample as a new subpath, decoded-pixel difference within recorded thresholds, and native output never bridges or reconnects across a gap | 3B-cont.+1 | v1 quality | `AT-FUNC-NAN-GAP` | Implemented (bounded Phase-3B local contract evidence: PR #105 + PR #106 F1; `TestCommittedGapAxesFixture`, `TestGapAdapterSemantics` in `test_agg_oracle_gap.py` plus `TestLine2DGapNativeAggParity` in `test_agg_oracle_line2d.py` — §5.4 Agg-oracle gap fixtures pass (interior NaN, leading/trailing non-finite, masked arrays, infinities), F1 24.8 cell-coverage rasterizer eliminates 172 fringe mismatches to zero for simple butt/miter strokes; adapter-scoped gap rendering path only, native Phase-1 series scope stays open per §15.1; release claim stays open) |
| `LP-MPL-021` | `MUST` | strict-preflight soundness — strict mode refuses every Figure it cannot represent completely and returns no PNG: any axes whose scale class is outside the declared eligibility surface (today: non-linear x/y scales), any inverted limits pair, and any geometry-stage refusal that leaves an axes with zero projected content produce an explicit `LumenPlotUnsupportedError`, never a successful partial or background-only frame; hybrid mode's whole-frame fallback diagnostic path remains the only success route for such frames | 3B-cont. | v1 quality | `AT-MPL-PREFLIGHT-SOUNDNESS` | Implemented (bounded Phase-3B local contract evidence: PR #103; `TestStrictPreflightSoundness`, `TestHybridPreflightSoundness` in `test_mpl_preflight_soundness.py` — strict mode refuses every unrepresentable Figure with explicit `LumenPlotUnsupportedError` and no PNG, hybrid fallback remains the only success route; adapter-scoped only, native Phase-1 rows stay open per §15.1 dual meaning; release claim stays open) |
| `LP-MPL-022` | `MUST` | public unit-conversion consumption — unit-aware artists (dates, calibrated units) are rendered from Matplotlib's publicly converted numeric data (the `orig=False` view) so converted geometry matches Agg's drawn geometry, while Figure-authoritative limits/converter semantics stay untouched; conversion outputs that cannot be represented natively are refused explicitly, never silently dropped to zero commands | 3B-cont.+1 | v1 quality | `AT-MPL-UNIT-DATA` | Not implemented (bounded Phase-3B local contract evidence: PR #85; `TestCalibratedUnitGeometry`, `TestUnrepresentableUnitData` in `test_phase3b_unit_data.py` with the date-lane upstream route `TestUnitConversionUpstream` in `test_phase3b_date_axis.py` (PR #74) — eligible geometry read through the public `orig=False` processed getters with Figure limits and converter authority preserved, unrepresentable payloads refused explicitly with no frame; calibrated-unit Agg pixel parity, full converter coverage, and release claims stay open) |
| `LP-MPL-023` | `SHOULD` | native collection translation — supported members of `LineCollection`, `PatchCollection`, and `PathCollection` are either translated natively with per-element style, clip, transform, and z-order fidelity (each class landing as an eligibility extension per `LP-MPL-020`: whitelist entry, collector-trace expectation, style contract, fixtures together) or refused explicitly; silent whole-class approximation is forbidden | W-lane, post-W1 | post-v1 | `AT-MPL-COLLECTIONS` | Not implemented |
| `LP-FUNC-041` | `SHOULD` | scalar-mappable semantics — where mappable artists become eligible (scatter/images per the post-v1 map), normalization and colormap application must match Matplotlib: `Normalize` (and declared alternatives) mapping, under/over/bad colors, and shared colorbar↔artist consistency — quality oracle: current Matplotlib Agg backend output; `AT-FUNC-MAPPABLE` is met only under the §5.4 criteria; owns artist-side mapping semantics only (native colorbar keeps its recorded disposition) | 5 | v1 non-blocking | `AT-FUNC-MAPPABLE` | Not implemented |
| `LP-FUNC-042` | `MUST` | native event-loop launch, window present, and close tests | 3 | v1 | `AT-FUNC-VIEWER`, `AT-LIFE-VIEWER`, `AT-LIFE-RUNTIME` | environment required — Not implemented (accepted 2026-09-13: proposal t_53261829, acceptance t_2db160d8; bounded partial evidence only: M4-B1 private headless viewer/runtime/surface seams with no present claim, plus the headless witness recorded on `LP-PLAT-010`/`LP-QUAL-020`; product launch, repeated cycles, real present, declared-cell matrix, and the ADR-0005 remainder still open; no support, performance, or release claim) |
...
| `LP-FUNC-003` | `MUST` | `AT-FUNC-AXIS` | Not implemented (bounded Phase-3B local contract evidence: PR #63; decorated-axes decoration rendering per ADR 0015 §4a — `TestDecoratedAxesSpec`, `TestDecoratedAxesEligibility`; date formatting remains unclosed; PR207 extension (B-2a R2): `TestAxisLabelWhitelist`, `TestAxisLabelDrawOrder`, `TestAxisLabelStyleContract` in `tests/python/test_phase3b_axislabels.py` plus `TestCommittedAxisLabelFixture` and `TestAxisLabelAdapterSemantics` in `tests/python/test_agg_oracle_axislabels.py` with `tests/python/generate_agg_oracle_axislabels.py --check`, fixture `axis-labels-xlabel-ylabel-axes-box-spines` under the UNCHANGED fixed S15.1 contract (background byte-equal, fully-covered byte-equal, fringe max channel delta 1, total-pixel fringe mismatch rate <= 0.001), deliberately no strict native pixel gate) |
| `LP-FUNC-004` | `MUST` | `AT-FUNC-LOG-AXIS` | Implemented (bounded Phase-3B local contract evidence: W3 lane t_a239680f; base-10 log-axis eligibility, projection, clip-rule domain handling, and Agg-oracle parity fixtures in `test_phase3b_log_axis.py`; non-base-10 log scales stay refused, release claim stays open; PR153 extension: `TestCommittedLogAxisFixture` and `TestLogAxisNativeAggParity` in `tests/python/test_agg_oracle_log.py` plus `tests/python/generate_agg_oracle_log.py --check`, fixture `log-axis-base10-line-decades-span`, fixed S15.1 contract (background byte-equal, fully-covered byte-equal, fringe max channel delta 1, total-pixel fringe mismatch rate <= 0.001), joined-oblique route b2004ee, merge f3bd279, CI 7/7) |
...
| `LP-FUNC-008` | `MUST` | `AT-FUNC-HISTORY` | Implemented (bounded Phase-1B local contract evidence: canonical-view restore, stability under interaction, and no-op/abort fixtures in `crates/lumenplot/tests/history.rs`; gesture routing and Previous/Next traversal remain open) |
...
| `LP-FUNC-025` | `MUST` | `AT-FUNC-3D` | Not implemented (bounded M2.5 native/adapter contract evidence: PR #116; `TestCommitted3DOracleFixtures`, `Test3DAdapterNativeSpec`, and `Test3DRefusalAndFallback` pass in the CPython 3.11 and 3.13 CI runs; full four-part v1 closure, platform matrix, and release claim remain open) |
...
| `LP-PROD-011` | `MUST` | `AT-REVIEW-FRAME-BOUNDARY` | Implemented (bounded M2 local contract evidence: PR #115; `SemanticFrame` is a distinct shared semantic/layout source for the validated `RenderPacket`, and `packet::tests::independent_consumers_read_one_validated_semantic_frame` passes in the CI Rust gate; broader semantic-frame integration remains open) |
| `LP-PROD-012` | `MUST` | `AT-REVIEW-PACKET` | Implemented (bounded M2 local contract evidence: PR #115; the doc-hidden `__internal` owner seam, immutable whole-packet validation, distinct scene/work/device generations, stale rejection, and root-export/public-constructor guards pass in CI; full property, renderer, and environment coverage remain open) |
...
| `LP-PLAT-001` | `MUST` | `AT-LIFE-PLATFORM` | Implemented (bounded M3 environment-cell evidence: offscreen validated-owner harness passes on the Radeon RADV PHOENIX executed cell and the Lavapipe control cell at 1x/1.25x/2x/3x oracle scales; pixel tolerance stays OPEN and window/present/lifecycle/support/performance/release claims remain open) |
...
| `LP-PLAT-010` | `MUST` | `AT-LIFE-RUNTIME` | environment required — Not implemented (bounded witness only: t_fafb7de4 headless 12-row logical outcomes plus 7 shm health transients at 34753814 per t_8a014b73 audit; status unchanged; product launch, repeated cycles, real present, and ADR-0005 remainder still open; no support, performance, or release claim) |
| `LP-QUAL-010` | `MUST` | `AT-SEC-SHADER` | Implemented (bounded M3 Phase-B local contract evidence: PR #118; build-time naga parse plus full-capability validation with fail-closed manifest pins, passing in the CI Rust gate; native-form and multi-artifact coverage remain open) |
| `LP-QUAL-011` | `MUST` | `AT-SEC-SHADER` | Implemented (bounded M3 Phase-B local contract evidence: PR #118; authoritative `shaders/manifest.toml` with runtime drift refusal, passing in the CI Rust gate; multi-artifact provenance remains open) |
| `LP-QUAL-012` | `MUST` | `AT-SEC-SHADER` | Implemented (bounded M3 Phase-B local contract evidence: PR #118; declared line class validated at build time and before GPU use, passing in the CI Rust gate; additional classes remain open; line-class GPU execution recorded on the Radeon executed cell and the Lavapipe control with pixel tolerance OPEN) |
...
| `LP-SEC-003` | `MUST NOT` | `AT-SEC-SHADER` | Implemented (bounded M3 Phase-B local contract evidence: PR #118; static runtime-download negative guard passing in the CI Rust gate; broader runtime-surface coverage remains open) |
| `LP-SEC-004` | `MUST` | `AT-SEC-SIZES` | Not implemented (bounded M2 local contract evidence: PR #181, merged as 1f263b7; malformed/capacity packet-bounds tests and exact series/point bounds pass in the CI Rust gate; broader allocation/upload/indexing coverage and full v1 closure remain open) |
...
| `LP-MPL-003` | `MUST` | `AT-MPL-PROFILES` | Implemented (bounded Phase-3B local contract evidence: `TestProfileSelectionValues`, `TestStagedDefaultProfile`, `TestSelectionIsImmutable`, `TestSelectionDrivesDispatch`; hybrid is the default mapping to hybrid-explicit; canonical profile-name closure and accelerated-native stay open) |
...
| `LP-QUAL-018` | `MUST` | `AT-LIFE-CANCELLATION` | Not implemented (bounded M2 local contract evidence: PR #181, merged as 1f263b7; session-layer generation-scoped cancellation pin passes in the CI Rust gate; LOD/layout worker binding stays open per the accepted row-state recommendation and full v1 closure remains open) |
| `LP-QUAL-019` | `MUST NOT` | `AT-LIFE-FFI` | Not implemented |
| `LP-QUAL-020` | `MUST` | `AT-LIFE-RUNTIME` | Implemented (bounded M2 local contract evidence: PR #115; `PacketOwner` retains the CPU semantic frame and rebuilds owner packets after device-generation invalidation, with `tests::packet_owner_retains_cpu_frame_and_rebuilds_after_device_invalidation` passing in the CI Rust gate; environment-backed device/surface/present coverage and full v1 lifecycle closure remain open) (bounded witness extension: t_fafb7de4 headless TerminalOutOfMemory mapping plus DeviceLost and RecoveryFailed explicit outcomes at 34753814 per t_8a014b73 audit; status unchanged; environment-backed device and surface remainder still open; no support, performance, or release claim) |
...
| `LP-FUNC-032` | `MUST` | `AT-FUNC-FILL` | Implemented (bounded Phase-3B local contract evidence: PR #68; fill/fill-between eligibility with §5.4 Agg-oracle parity fixtures in `test_phase3b_fill.py`; extended by PR #161 stackplot-oracle fixture evidence (merged as c7f9f13, CI 7/7): `TestCommittedStackplotFixture`, `TestStackplotCollectorTrace`, `TestStackplotStyleContract`, `TestStackplotNativeAggParity` in `test_agg_oracle_stackplot.py` with generator `--check`, fixture_id `stackplot-two-layer-opaque`, fixed S15.1 contract (background and fully-covered byte equality, fringe max channel delta 1, total-pixel fringe mismatch rate <= 0.001); slanted-edge parity stays open; release claim stays open) |
| `LP-FUNC-033` | `MUST` | `AT-FUNC-BAR` | Implemented (bounded Phase-3B local contract evidence: PR #69 + PR #89 eligibility + PR #102 fixtures (merged as 24bde5a, parity green in CI with exact-class 0 mismatches) + PR #107 convergence; bar eligibility with §5.4 Agg-oracle parity fixtures in `test_phase3b_bar.py` plus `TestCommittedBarFixture`, `TestBarNativeAggParity` in `test_agg_oracle_bar.py`; release claim stays open) |
| `LP-FUNC-034` | `MUST` | `AT-FUNC-DRAWSTYLE` | Implemented (bounded Phase-3B local contract evidence: PR #70 + PR #111 convergence (c17ab3e merged as 4d49217, CI green; PR #109 fixture branch 68ddac3 superseded, unmerged); step-drawstyle eligibility with §5.4 Agg-oracle parity fixtures in `test_phase3b_steps.py`; release claim stays open) |
| `LP-FUNC-040` | `MUST` | `AT-FUNC-NAN-GAP` | Implemented (bounded Phase-3B local contract evidence: PR #105 + PR #106 F1; gap fixtures with §5.4 Agg-oracle parity in `test_agg_oracle_gap.py` plus `TestLine2DGapNativeAggParity` in `test_agg_oracle_line2d.py`; release claim stays open) |
| `LP-FUNC-042` | `MUST` | `AT-FUNC-VIEWER`, `AT-LIFE-VIEWER`, `AT-LIFE-RUNTIME` | environment required — Not implemented (accepted 2026-09-13: proposal t_53261829, acceptance t_2db160d8; bounded partial evidence only; product launch, repeated cycles, real present, declared-cell matrix, and the ADR-0005 remainder still open; no support, performance, or release claim) |
| `LP-MPL-021` | `MUST` | `AT-MPL-PREFLIGHT-SOUNDNESS` | Implemented (bounded Phase-3B local contract evidence: PR #103; strict-preflight soundness fixtures in `test_mpl_preflight_soundness.py`; release claim stays open) |
| `LP-MPL-022` | `MUST` | `AT-MPL-UNIT-DATA` | Not implemented (bounded Phase-3B local contract evidence: PR #85; converted-data geometry and explicit-refusal fixtures in `test_phase3b_unit_data.py` with the date-lane upstream route in `test_phase3b_date_axis.py`; Agg pixel parity beyond date frames and release claims stay open) |
...
| `LP-FUNC` | native functional interaction, annotations, viewer, optional capability scope, and the adopted Matplotlib major-feature rows | Phase-1 view/scale implementation and local evidence are recorded in [API 0001](../architecture/api-0001-native-scene-state.md); interaction, annotations, viewer, and v1 evidence remain pending. The bounded M2.5 3D native/adapter slice is CI-verified in PR #116, but the full `LP-FUNC-025` row remains `Not implemented`. `LP-FUNC-008` now carries bounded implemented evidence for the deterministic history-transition state substrate (canonical-view restore, stability under interaction, no-op/-0.0 equality, atomic validation) in `crates/lumenplot/tests/history.rs`, while gesture routing, Previous/Next traversal, and animated transitions stay open. The 2026-09-11 window-app scope proposal, accepted 2026-09-13, adds `LP-FUNC-042` for the interactive window application (native event loop, window/present, close) with `AT-FUNC-VIEWER`, `AT-LIFE-VIEWER`, and `AT-LIFE-RUNTIME` gates; M4-B1 headless seams are partial evidence only, and real present plus declared-cell closure stay open. The 2026-08-25 adoption added `LP-FUNC-032`–`LP-FUNC-039` (polygon fill, bars/histograms, step drawstyles, cross-primitive compositing, polar, date/unit axes, quiver, scale extensions) from [`matplotlib-parity-requirements-draft.md`](matplotlib-parity-requirements-draft.md); the W1 "static geometry" rows (`LP-FUNC-032`/`033`/`034`) now carry bounded implemented results (PRs #68/#69/#70, integrated by PR #71 with mixed-workload composition fixtures in `test_phase3b_integration_mixed.py`) whose §5.4 Agg-oracle parity gates are met within recorded thresholds, while the remaining added rows stay `Not implemented`, the flagged contract impacts in that document's Section 8 are hand-off notes for the architecture authority, not designs, and every gate above is additionally bound to that document's §5.4 quality oracle: acceptance requires Agg parity fixtures (geometry parity to sub-pixel rounding, decoded-pixel difference within recorded thresholds, style semantics equal to Agg's resolution; text via `TextPath` outline comparison). On 2026-08-26 the architecture authority adopted the GAP-R1 candidate rows `LP-FUNC-040` (non-finite sample fidelity) and `LP-FUNC-041` (scalar-mappable semantics) from [`codex-gap-verification-candidate-rows.md`](../research/codex-gap-verification-candidate-rows.md); both remain `Not implemented` pending their §5.4-bound gates (`AT-FUNC-NAN-GAP`, `AT-FUNC-MAPPABLE`). |
...
| `LP-PLAT` | capability selection, main-thread runtime, surfaces, device loss, and platform matrix | `LP-PLAT-001` carries bounded M3 environment-cell evidence (offscreen validated-owner harness on the Radeon executed cell and the Lavapipe control, tolerance OPEN); main-thread runtime, surfaces, device loss, lifecycle matrix, platform matrix, and all support/performance/release claims remain pending with no completion claimed. |
| `LP-PROD` | product independence, shared semantic frame, and core dependency direction | Phase-1 native core/facade implementation and local evidence are recorded in [ADR 0010](../adr/0010-phase1-native-core-facade-contract.md); bounded private packet/resource-lifecycle model evidence is recorded in PR #91, and PR #115 adds the shared `SemanticFrame` plus the validated owner packet handoff, giving `LP-PROD-011`/`LP-PROD-012` bounded implemented results; PR #181 (merged as 1f263b7) records the explicit EXIT-1 not-to-wire for export plus bounded generation-ordering evidence; broader shared-frame integration and product evidence remain pending. |
| `LP-QUAL` | hot path, lifecycle, shaders, dependency gates, repository boundary, and release quality | Phase-1 Scene ownership/publication implementation and local evidence are recorded in [API 0001](../architecture/api-0001-native-scene-state.md); PR #91 adds bounded logical resource-lifecycle model tests, PR #115 adds bounded owner-side retained-frame/device-generation rebuild evidence for `LP-QUAL-020`, PR #118 adds bounded line-only shader build/manifest/validation evidence for `LP-QUAL-010`/`011`/`012`, with the declared line class additionally executed on the Radeon cell and the Lavapipe control (tolerance OPEN), and PR #181 (merged as 1f263b7) adds the session-layer generation-scoped cancellation pin for `LP-QUAL-018` with LOD/layout worker binding staying open; hot-path, environment-backed lifecycle, multi-artifact shader, and release evidence remain pending. |
...
| `LP-SEC` | unsafe boundary, runtime shader policy, input validation, provenance, SBOM, fonts, and public docs | PR #118 adds bounded line-only runtime-negative-guard and artifact-verification evidence for `LP-SEC-003`/`005`, and PR #181 (merged as 1f263b7) adds bounded malformed/capacity packet-bounds evidence for `LP-SEC-004` with broader allocation/upload/indexing coverage staying open; broader runtime/provenance/SBOM/font/docs evidence remains pending. |
...
- On 2026-09-05 the maintainer separately promoted `LP-FUNC-025` to a Phase 3 v1 `MUST`. Its `AT-FUNC-3D` acceptance is the canonical §15.1 four-part parity contract applied to pinned `FigureCanvasAgg` mplot3d output, including reference rendering artifacts rather than a superiority claim; PR #114 records the O-19–O-22 decisions, and PR #116 now supplies bounded CI-verified M2.5 evidence in the registry, while the full-row result remains `Not implemented`.
- The 2026-09-11 window-app scope proposal, accepted 2026-09-13, adds `LP-FUNC-042` (interactive window application: native event loop, window/present, close; Phase 3 v1 `MUST`; `AT-FUNC-VIEWER`, `AT-LIFE-VIEWER`, `AT-LIFE-RUNTIME`) as accepted scope; it complements `LP-FUNC-016` and `LP-PLAT-009`/`LP-PLAT-010` without changing any existing row, and its transport/host scope (declared-cell-first vs matrix) plus Q1–Q5 remain open questions in the accepted ADR 0005 note and the M4 roadmap placement.
