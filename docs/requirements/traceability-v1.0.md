...
- The 2026-09-05 envelope amendment promotes `LP-FUNC-025` to a Phase 3 v1 `MUST` under [ADR 0016](../adr/0016-v1-3d-envelope-and-agg-parity.md). It remains honestly `Not implemented` as a full v1 requirement; the bounded M2.5 native/adapter evidence is recorded below, and whole-frame Agg fallback alone is not native 3D evidence.
...
| v1 3D envelope and Agg-parity gate | [ADR 0016](../adr/0016-v1-3d-envelope-and-agg-parity.md) | Accepted product envelope and O-19–O-22 amendment; bounded M2.5 native/adapter evidence is recorded below; full `LP-FUNC-025` remains `Not implemented` |
| M2.5 bounded 3D native/adapter parity slice | [ADR 0016](../adr/0016-v1-3d-envelope-and-agg-parity.md) + `tests/python/test_agg_oracle_3d.py` + render-api owner-boundary tests | Three pinned perspective/orthographic fixtures, public `Line3D`/`Poly3DCollection` preflight, fixed-cell triangle/stroke parity, explicit refusal/fallback, and additive semantic-frame evidence pass locally; full requirement, platform matrix, and release closure remain open |
